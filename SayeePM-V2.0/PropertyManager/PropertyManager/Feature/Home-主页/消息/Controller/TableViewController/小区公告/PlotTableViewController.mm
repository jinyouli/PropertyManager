...
-(void)getDataFromNetWork{
    ...
    if ([self checkNetWork]) {
        ...
        NSDictionary * paraDic = @{@"notice_type":@"2",@"department_id":department_id,@"current_page":@(self.page),@"page_size":@"5"};
        ...
        [DetailRequest SYGet_notice_by_pagerWithParms:paraDic SuccessBlock:^(NSArray *value) {
            ...
            for (PlotModel * model in modelArr) {
                ...
                if (res == 1) {
                    BOOL res2 = [self isReadFromDBWithNoticeID:model.noticeID];
                    model.state = res2;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    MyFMDataBase * database = [MyFMDataBase shareMyFMDataBase];
    NSDictionary * params = @{@"fusername":[UserManagerTool userManager].username,@"noticeID":model.noticeID};
