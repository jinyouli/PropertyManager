...
- (void)settingData
{
    ...
    if (type == 1 || type == 2) {
        ...
        if ([PMTools isNullOrEmpty:self.replyDataModel.fcontent]) {
            self.desLabel.text = [NSString stringWithFormat:@"该工单(%@号)处理人已有[%@]变更为[%@]",_replyDataModel.ID,_replyDataModel.old_name,_replyDataModel.nMyName1];
        }
        else{
            self.desLabel.text = [NSString stringWithFormat:@"该工单(%@号)处理人已有[%@]变更为[%@],原因%@",_replyDataModel.ID,_replyDataModel.old_name,_replyDataModel.nMyName1,self.replyDataModel.fcontent];
        }
    }
    else if (type == 5){
        self.nameIcon.text = @"系统";
